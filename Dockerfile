...
ENTRYPOINT ["java" ,"-cp","app:app/lib/*","com.peertutor.StudentMgr.StudentMgrApplication"]
CMD ["-Dspring.profiles.active=docker"]
# to use for ci cd
# ENTRYPOINT ["java" ,"-cp","app:app/lib/*","com.peertutor.StudentMgr.StudentMgrApplication", "-Dspring.profiles.active=docker"]
